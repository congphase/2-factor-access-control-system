    ...
    ********************************************************************************************************************
    ...
    while True:
        ...
        if process_this_frame:
            ...
            if candidate_id == "":
                log_debug('id not passed yet')
            else:
                candidate_id = int(candidate_id)
                candidate_name = tnt_info.tnt_name_tup[candidate_id]
                log_debug(f"id passed: {candidate_id}, it's {candidate_name}")
            ...
            if len(bboxes) == 0:
                ...
                imshow(tegra_cam.WINDOW_NAME, frame)
                ##movie_write(frame)
                continue
            ...
            ###############################################
                ...
                log_debug(f'(c | l, t, r, b) = ({bb_conf:.2f}, '
                ...
                if type(candidate_id) is int:
                    log_debug("got id, starting verification")
                    ...
                    freq_cv.stick_name(frame, (bb_left, bb_top, bb_right, bb_bottom), candidate_name)
            ...
            ###############################################
                ...
                if type(candidate_id) is int:
                    ...
                    if matches.count(True) > (len(matches) / 2):
                        print(MSG_XAC_THUC_KHUON_MAT_THANH_CONG)
                        log_info(f'Xac thuc khuon mat thanh cong: (id: {candidate_id}; name: {candidate_name})')
        ...
        cv2.imshow(tegra_cam.WINDOW_NAME, frame)
        cv2.waitKey(10)
