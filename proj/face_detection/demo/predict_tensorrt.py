...
def run_prediction_folder():
    ...
    data_folder = '/home/gate/lffd-dir/proj/face_detection/demo/'
    file_name_list = [file_name for file_name in os.listdir(data_folder) \
                        if file_name.lower().endswith('mp4')]

    onnx_file_path = "/home/gate/lffd-dir/proj/face_detection/deploy_tensorrt/onnx_files/v2_small.onnx"
    ...
    for file_name in file_name_list:
        ...
        while cv2.waitKey(1) < 0:
            ...
            tic = time.time()
            bboxes = myInference.do_inference(frame, score_threshold=0.6, top_k=1000, NMS_threshold=0.2, NMS_flag=True)
            toc = time.time()
            detect_time = (toc - tic) * 1000
            timeRecSave.append(detect_time)
        ...
        plt.plot(timeRecSave, label='detection time')
        plt.ylim([0, 82])
        plt.xlabel('Frame'),
        plt.ylabel('Detection time (ms)'),
        plt.title('Model da duoc toi uu voi TensorRT')
        plt.hlines(y=timeRecMean, xmin=0, xmax=frame_count,
                linewidth=2, color='r', label='Mean')
        plt.savefig('optimized.png', bbox_inches='tight')
        plt.legend(loc='upper center', bbox_to_anchor=(0.5, 0.5), shadow=True, ncol=2)
        plt.show()
