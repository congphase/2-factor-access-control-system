...
def main():
    ...
    print("Predict Constructor time: {:.3f}s".format(end - start))
    ...
    if args.mode == 'image':
        ...
        print("Get files time: {:.3f}s".format(end - start))
        ...
        for file_name in file_name_list:
            ...
            plt.title('Model ONNX chua toi uu')
            plt.hlines(y=timeRecMean, xmin=0, xmax=frame_count,
                    linewidth=2, color='r', label='Mean')
            plt.savefig('non-optimized.png', bbox_inches='tight')
