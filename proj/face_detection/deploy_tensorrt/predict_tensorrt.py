...
def run_prediction_folder():
    ...
    for file_name in file_name_list:
        ...
        logging.info('bboxes looks: %s', bboxes)
        logging.info('this is to know at what index x1 stays. Take a look at it')
        '''
        tmp_list = []
        for i, bbox in enumerate(bboxes):
            logging.debug('bboxes[%d] = %s', i, bbox)
            tmp_list.append(bbox[0])
        logging.debug('tmp_list = %s', tmp_list)
        logging.debug('min of x1 value: %s', str(min(tmp_list)))
        logging.debug('index of min of x1 values = %s', bboxes.index(tmp_list[min(tmp_list)]))
        '''
        for bbox in bboxes:
