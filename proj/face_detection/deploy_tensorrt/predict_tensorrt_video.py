...
def run_inference(video_in, video_out, anchor_conf, candidate_id, candidate_known_encs):
    """
    ...
    while movie_isOpened():
        ...
        if process_this_frame:
            ...
            if len(bboxes) == 0:
                ...
                log_debug(f'(bb_conf; type) = ({bb_conf}; {type(bb_conf)}')
                log_debug(f'(anchor_conf; type) = ({anchor_conf}; {type(anchor_conf)})')
                ...
                if matches.count(True) > (len(matches) / 2):
                    ...
                    # for debug purpose
                    frame = cvtColor(frame, COLOR_RGB2BGR)
                    imwrite(f'left_most_{bb_left}.jpg', frame[bb_top:bb_bottom, bb_left:bb_right])
...
def main():
    ...
    candidate_known_encs = tnt_info.get_smpl_encs(candidate_id, current_time)

    run_inference(args.video_in, args.video_out, 1.7, candidate_id, candidate_known_encs)
    run_inference()
