...
test_img = cv2.imread("/home/gate/lffd-dir/A-Light-and-Fast-Face-Detector-for-Edge-Devices/face_detection/deploy_tensorrt/0_1090.jpg")
#get_face_encodings = face_recognition.face_encodings
get_face_encodings = my_api.face_encodings
...
"""
tic = time.time()
face_encoding = get_face_encodings(test_img_dlib, css_type_face_location, 0)[0]
toc = time.time()
"""
dlib_rectangle_object = dlib.rectangle(bb_top, bb_left, bb_right, bb_bottom)

shape = my_api.pose_predictor_5_point(test_img_dlib, dlib_rectangle_object)

face_chip = dlib.get_face_chip(test_img_dlib, shape)

face_encoder = my_api.face_encoder

face_encoding = face_encoder.compute_face_descriptor(face_chip)

print(f'[dlib] face_encoding:\n{face_encoding}')

#print(f'time: {toc - tic}s')

np_array_encoding = numpy.array(face_encoding)

print(f'numpy array encoding:\n{np_array_encoding}')
...
tic = time.time()
face_encoding = get_face_encodings(test_img_lffd, css_type_face_location, 0)[0]
toc = time.time()
